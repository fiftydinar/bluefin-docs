...
test("PortalNews statically renders the build-time blog index", () => {
  ...
  assert.ok(
    !html.includes('<a href="/blog" target="_blank"'),
    "internal /blog view-all link must not open in a new tab",
  );
});
...
test("PortalNews first render is the same with and without a window when the index is empty", () => {
  // A genuinely empty index (not unavailable) produces "no posts" on both
  // server and client without hydration mismatch.
  const PortalNews = loadPortalNews({ posts: [] }).default;
...
test("PortalNews renders unavailable state with stateReason on first render without hydration mismatch", () => {
  // The unavailable-index case must visibly report unavailability with its reason
  // on both server and client without hydration mismatch.
  const reason = "Could not read blog directory: ENOENT";
  const PortalNews = loadPortalNews({
    unavailable: true,
    stateReason: reason,
    posts: [],
  }).default;
  ...
  assert.equal(clientHtml, serverHtml);
  assert.ok(
    serverHtml.includes(`Latest posts are unavailable (${reason}).`),
    "must render unavailable state with stateReason",
  );
  assert.ok(
    !serverHtml.includes("No blog posts found."),
    "must not render generic no-posts message when reason is provided",
  );
  assert.ok(!serverHtml.includes("Loading blog posts..."));
});
...
test("normalizeBlogIndex preserves stateReason and unavailable flag", () => {
  const { normalizeBlogIndex } = loadPortalNews();

  assert.deepEqual(
    normalizeBlogIndex({
      unavailable: true,
      stateReason: "Blog dir missing",
      posts: [],
    }),
    { posts: [], unavailable: true, stateReason: "Blog dir missing" },
  );
  ...
  assert.deepEqual(normalizeBlogIndex(null), {
    posts: [],
    unavailable: false,
  });
  assert.deepEqual(
