...
1. **PR Created** → Workflow creates PR from feature branch
2. **Auto-merge Enabled** → `gh pr merge --auto` sets auto-merge flag
3. **Merge Queue** → PR enters queue (waits 5 min minimum per ruleset)
4. **Status Checks** → All required checks must pass (ALLGREEN strategy)
...
✅ **Build validation:** Merge queue tests PR before merging
✅ **Status checks:** ALLGREEN strategy requires all checks pass
✅ **Audit trail:** PR creates reviewable record
✅ **Rollback capability:** PR can be reverted via GitHub UI

### What Auto-Merge Does NOT Bypass

🔒 **Branch protection rules** - Still enforced
🔒 **Required status checks** - Must pass
🔒 **Merge queue** - PR must clear queue
🔒 **Merge conflicts** - PR fails if conflicts exist
