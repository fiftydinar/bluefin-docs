...
To stop Project Bluefin countme reporting now and across reboots, mask its systemd timer and service:

```bash
sudo systemctl mask --now bluefin-countme.timer bluefin-countme.service
```
