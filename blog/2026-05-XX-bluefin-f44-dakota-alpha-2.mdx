...
- **Unified Kernel Images (UKI)** — kernel, initrd, and command line bundled and signed for Secure Boot
- **composefs with fs-verity** — every file in the OS image is cryptographically verified at read time, managed by bootc
- **Full verified boot chain** from firmware → bootloader → kernel → OS image, all Secure Boot signed
- **TPM-backed passwordless disk unlocking** — the main user-visible benefit; unlocking is tied to the verified boot state
