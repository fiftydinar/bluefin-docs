...
<BlogFigure
  ...
  caption="OGC Gaming kernel and stack on Bluefin Dakota"
/>
...
Let's add a new application. Let's say the control panel we've all been missing. [Brian Ketelsen](https://brian.dev/), Father of Bluefin and now control panel connoisseur has blessed us with this:  
...
He's been rolling with this in his custom image. And if you're like me you probably want that thing right now. Here's the [Git repo](https://github.com/frostyard/chairlift). Not distribution specific either, Brian's images are based on Debian. See how weird it is to classify things the old way? Who cares. It doesn't even ship on the image it ships in homebrew lol. 
...
- Contributor: edit an image-specific build script (e.g., `build/10-build.sh` executing [`dnf5`](https://github.com/rpm-software-management/dnf5) `install -y <package>`) or a `bst` definition file.
